...
public class ResidencialRuralAE extends Pagante_Agua {
    public ResidencialRuralAE(int consumo){
        super(consumo);
        setTarifaResidencialRural();
        
    }
    
    private void setTarifaResidencialRural(){
            int faixa = getConsumo();
            if (faixa <=10){
                super.setTarifa(TarifaAE.RURAL_0_A_10);
            }
            else if ((faixa >=11) && (faixa <=20)){
                super.setTarifa(TarifaAE.RURAL_11_A_20);
            }
            else if ((faixa >=21)&&(faixa <=30)){
                 super.setTarifa(TarifaAE.RURAL_21_A_30);
            }
            else if ((faixa >=31 )&&(faixa <=50)){
                 super.setTarifa(TarifaAE.RURAL_31_A_50);
            }
            else 
                super.setTarifa(TarifaAE.RURAL_51);
        }
     @Override
