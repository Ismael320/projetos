...
public class ResidencialAE extends Pagante_Agua {
    
   public ResidencialAE(int consumo){
        super(consumo); 
        setTarifaResidencialComum();
    }
        
    private void setTarifaResidencialComum(){
            int faixa = getConsumo();
            if (faixa <=10){
                super.setTarifa(TarifaAE.RESIDENCIAL_0_A_10);
            }
            else if ((faixa >=11) && (faixa <=20)){
                super.setTarifa(TarifaAE.RESIDENCIAL_11_A_20);
            }
            else if ((faixa >=21)&&(faixa <=30)){
                 super.setTarifa(TarifaAE.RESIDENCIAL_21_A_30);
            }
            else if ((faixa >=31 )&&(faixa <=50)){
                 super.setTarifa(TarifaAE.RESIDENCIAL_31_A_50);
            }
            else 
                super.setTarifa(TarifaAE.RESIDENCIAL_51);
        }
